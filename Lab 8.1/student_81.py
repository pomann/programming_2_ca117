...
class Student(object):
    ...
    def add_mark(self, c, m):
        self.__marks[c] += int(m)

    def precision_mark(self):
        opt = 0
        s = 0
        for i in self.__marks:
            if self.__mods[i][2] == 10:
                opt += int(self.__marks[i]) * 2
                s += 2
            else:
                opt += int(self.__marks[i])
                s += 1
        return opt / s
    ...
    def passed_by_compensation(self):
        m = self.precision_mark()
        s = 0
        for i in self.__marks:
            if int(self.__marks[i]) < 40:
                if int(self.__marks[i]) < 35:
                    return False
